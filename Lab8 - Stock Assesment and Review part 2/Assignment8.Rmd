...
Subset for Menhaden catch 1977 - 1982 to follow the same cohort.
```{r}
m =15
C = as.numeric(menhaden[14,][3])
for (i in 4:8) {
C = append(C,as.numeric(menhaden[m,][i]))
m=m+1
}
C
```
...
*BONUS*
3) Modify the `getYpR` function for the Menhaden analysis you conducted in questions 1-2. What is the optimal fishing mortality rate for maximum YPR? How does this YPR analysis compare with the initial fishing mortality rate of 0.6? Could they have fished more, or did they need to fish less to achieve optimal YPR? 
