...
1) Use the Catch-at-Age data below to simulate a population of vulnerable recruits `assuming fishing mortality = 0.2 AND is constant.`
...
Subset menhaden catch to view the population structure and catch of 1982 (remove age=0 because they are not vulnerable to fishing gear).
```{r}
...
3) Modify the `getYpR` function for the Menhaden analysis you conducted in questions 1-2. What is the optimal fishing mortality rate for maximum YPR (assuming recruitment is constant)? How does this YPR analysis compare with the initial fishing mortality rate of 0.2? 
```{r}
getYpR = function(Fi){
  N0 = 100
  years = 1:10
