...
VPA uses catch data (fishery-dependent data collection) to determine past stock size using mortality rates (Jennings et al., 2001). It uses fishing mortality (number landed) and natural mortality (estimated) to discern the number in the stock for the previous time period (i.e. the previous year/month) and survivors.

$$
N_{t+1} = N_te^{-(F+M)}
$$

Where F and M are instantaneous rates of Fishing and Natural mortality. We can then calculate the number fish caught with the catch equation:
...
$$
N_t = \frac{C_t}{(F_t/Z_t)(1-e^{-Z_t})}\\t=4
$$

Generate catch data for 4 years
```{r}
C = sort(runif(4, min=30, max=100), decreasing=T) # generates 4 random catch values 
C
```
...
Define a function to calculate fishing mortality from the above catch equation
```{r}
...
$$
N_t = \frac{C_t}{(F_t/Z_t)(1-e^{-Z_t})}\\t=8
$$
...
# Yield-per-Recruit
Yield per Recruit is the expected life time yield per fish recruited into the stock at a
