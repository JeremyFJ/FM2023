...
```{r}
matrix(1:9, nrow = 4, ncol = 4) # range, number of rows, number of columns
```
...
The `rfisheries` package provides access to the Open Fisheries global fisheries data - including the total annual number of *landings* for each year. The `FSA` package records biological fisheries data from the Global Fisheries Stock Assesment organization. 
```{r}
...
```{r}
head(of_country_codes()) # `head()` prints the top of the output, and leaves out the rest
```
...
```{r}
# par(mfrow=c(1,2))
BlackTip = of_landings(species = "CCL")
Sturgeon = of_landings(species = "STU")
CCL_STU <- rbind(BlackTip, Sturgeon)
ggplot(CCL_STU) +
  geom_line(aes(x=year, y=catch)) +
  ylab("Annual Catch (Tons)")+
  xlab("Year")+
  ggtitle("Global Blacktip shark and Sturgeon landings") + theme_bw()
summary(BlackTip)
```
