...
y = m*x + b # now we generate a series of response values without error
err2 = rnorm(20,mean=mu,sd = sigma) # let's assume the error is normally distributed with mean 0 and sd  = 5
y = y+err2 # add error to mean
```
...
b_list = seq(0.1,10,0.1)
m_list = seq(0.1,10,0.1)
params = matrix(NA,nrow=100,ncol=100,dimnames = list(b_list,m_list))
for (i in 1:length(b_list)){ # for b
 	for (j in 1:length(m_list)){ # for m
...
`?optim`
```{r}
pars = c(b = b, m = m, err.sigma = sigma)
