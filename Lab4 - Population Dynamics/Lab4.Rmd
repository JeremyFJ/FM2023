...
top5_iattc <- subset(iattc, year >= 1990)
top5_iattc <- subset(top5_iattc, fleet=="CHN" | 
                       fleet == "ESP" | 
                       fleet == "JPN" | 
                       fleet == "KOR" | 
                       fleet == "TWN")
iattc_plot = ggplot(top5_iattc, aes(fill=fleet, y=catch*0.001, x=year)) + # top fleets with most catch
  geom_bar(position="stack", stat='identity') + ggtitle("IATTC Swordfish") + 
  ylab("Catch (t)")

# WCPFC
fleet_wp = aggregate(data=wcpfc, catch~fleet, sum) # aggregate catch by fleet
top5fleets =  fleet_wp[order(-fleet_wp$catch),] # order fleet by top catches
top5_wcpfc <- subset(wcpfc, year >= 1990)
top5_wcpfc <- subset(top5_wcpfc, fleet=="JP" | 
                       fleet == "TW" | 
                       fleet == "CN" | 
                       fleet == "KR" | 
                       fleet == "AU")
