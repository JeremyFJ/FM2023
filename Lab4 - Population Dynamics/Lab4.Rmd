...
ggplot() + 
  ...
  scale_fill_gradientn(colors = hcl.colors(7), 
                       limits=c(0,4)) +
  xlim(60,180) 
```
...
## Catch Curves, Total Mortality, and Length-Frequency Distribution
...
perch2000 %<>% mutate(lcat10=lencat(length,w=10))
...
```{r}
perch %<>% mutate(lcat10=lencat(length,w=10))
```

```{r}
vb = vbFuns(param="Typical") # from FSA package
f.starts <- vbStarts(length~age,data=RedDrum) # vbStarts is an optimization algorithm designed to find the best params
